// state.js — engine-level state for SEED / Ruina Liminalis (bible §7).
// The reusable spine: flags, XP (life-gives-XP), persistent companions, the verb,
// save/load, the portal handoff — PLUS the original through-line this file always
// carried: the persisting verb's witnesses (played), the year/age counter, scores
// carried forward, and the command-line log that demotes but never leaves.
// Vanilla ES module. No build step. Import once; share across every era and seam.
//
//   import { state, flags, xp, party, verb, save, load } from './state.js';
//
...
/* ------------------------------------------------------------------ *
 * 0. THE THROUGH-LINE — played witnesses, year, scores, the log
 *    (pre-spine exports; existing modules import these — keep stable)
 * ------------------------------------------------------------------ */

export const state={played:{ballistic:false,asteroid:false,tanks:false},year:1979,scores:{}};

// anyPlayed() is no longer the menu gate (flags are), but kept for compat.
...
// Autosave hook: main.js registers ()=>save() for real (non-admin) sessions.
// flags.set calls it AFTER a first-set's hooks have run, so the persisted blob
// already reflects whatever the hooks changed.
let _autosave = null;
export function autosaveOn(fn) { _autosave = fn; }
...
export const flags = {
  ...
  set(name, value = true) {
    ...
    if (firstTime && _autosave) _autosave();
    return firstTime;
  },
...
export const FLAG = Object.freeze({
  // --- Era 0 (The Dot) — goal gates; the select menu grows on these ---
  ERA0_BALLISTIC_DONE:     'era0_ballistic_done',
  ERA0_ASTEROID_DONE:      'era0_asteroid_done',
  ERA0_TANKS_DONE:         'era0_tanks_done',
  // --- Limina I ---
...
export function save(slot = 'seed') {
  const blob = {
    ...
    party: party.members(),
    verb: [...verb.caps],
    // the through-line rides in the same blob
    played: { ...state.played },
    year: state.year,
...
export function load(slot = 'seed', blob = null) {
  ...
  // the through-line restores IN PLACE — other modules hold live references
  if (data.played) Object.assign(state.played, data.played);
  if (typeof data.year === 'number') state.year = data.year;
  if (data.scores) Object.assign(state.scores, data.scores);
  if (Array.isArray(data.log)) { log.length = 0; data.log.forEach((s) => log.push(s)); }
  return true;
}
